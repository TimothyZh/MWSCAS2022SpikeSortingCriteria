...
def voltage_deg_model_sparse_conductance(G_mem, V_app_WL1, V_app_BL2, R_source, R_line):
    ...
    with open('ABCD.npy', 'wb') as f:
        np.save(f, ABCD_matrix.to_dense().numpy())

    with open('E.npy', 'wb') as f:
        np.save(f, E_matrix.to_dense().numpy())

    return torch.linalg.solve(ABCD_matrix.to_dense(), E_matrix.to_dense())
...
for idx, row in int_filter.iterrows():
    for i in range(1, 6):
        df = df.append(
            {'Alignment': row['Alignment'], 'Method': 'Integer Filter', 'Accuracy (%)': row['Dataset %d' % i] * 100}, ignore_index=True)

wavelet = pd.read_csv('wavelet.csv')
for idx, row in wavelet.iterrows():
    for i in range(1, 6):
        df = df.append(
            {'Alignment': row['Alignment'], 'Method': 'Wavelet', 'Accuracy (%)': row['Dataset %d' % i] * 100}, ignore_index=True)

df.loc[df['Alignment'] == 'Max Amplitude Alignment',
       'Alignment'] = 'Max Amplitude'
df.loc[df['Alignment'] == 'Max Derivative Alignment',
       'Alignment'] = 'Max Derivative'
df.loc[df['Alignment'] == 'No Alignment',
       'Alignment'] = 'None'
sns.boxplot(data=df, x="Alignment", y='Accuracy (%)', hue='Method',
            showfliers=True, palette=color_palette, ax=ax[0])
ax[0].set_ylim([70, 101])
ax[0].xaxis.grid(True, which='both')

ICV = pd.read_csv('ICV.csv')
df = pd.DataFrame(columns=['Method', 'Dataset',
                  'Accuracy (%)', 'ICV (Weighted)'])
for idx, row in ICV.iterrows():
    for i in range(1, 6):
        df = df.append(
            {'Method': row['Method'], 'Dataset': i, 'ICV (Weighted)': row['Dataset %d' % i]}, ignore_index=True)
...
for idx, row in int_filter.iterrows():
    for i in range(1, 6):
        df.loc[(df['Method'] == 'Integer Filter') & (df['Dataset'] == i), 'Accuracy (%)'
               ] = row['Dataset %d' % i] * 100
...
sns.barplot(data=df, x='Dataset', y='ICV (Weighted)',
            hue='Method', palette=color_palette, linewidth=1, edgecolor='k', ax=ax_left)
sns.barplot(data=df, x='Dataset', y='Accuracy (%)',
            hue='Method', hatch='xx', palette=color_palette, linewidth=1, edgecolor='k', ax=ax_right)

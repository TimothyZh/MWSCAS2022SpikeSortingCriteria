...
df = pd.DataFrame(columns=['Implementation', 'Scenario', 'Power (mW)',
...
peripheral_latency = (1 / ADC_frequency) * \
    ADC_bit_resolution * tiles.shape[0] * tiles.shape[2]
...
df = df.append({'Implementation': 'TDM', 'Scenario': '(R_ON + R_OFF) / 2', 'Power (mW)': total_power * 1e3, 'Area (mm2)': total_area * 1e6,
